...
parse_args ()
{
  ...
  while [ "$#" -gt 0 ]; do
      ...
      --static)
          DOCKER_MODE="static"
          debug_print "set DOCKER_MODE to ${DOCKER_MODE}"
          ;;
      -u|--upload)
          UPLOAD="yes"
...
auto_fill_args () {
  ...
  if [ -z "${TAG}" ]
  then
    TAG="${DOCKER_MODE}-${OS_NAME}-${OS_VER}-${PLATFORM_TAG}"
    CMD+=("-t ${DOCKER_PREFIX}:${TAG}")
  fi
  CMD+=("--build-arg DOCKER_BASE_IMAGE=${OS_NAME}:${OS_VER}")
  CMD+=("--build-arg DOCKER_ENV_SCRIPT=./docker/${DOCKER_MODE}/${OS_NAME}${OS_VER}.sh")
  CMD+=("-f ${CODE_DIR}/docker/${DOCKER_MODE}/linux.DockerFile ${CODE_DIR}")
}
...
check_args () {
  ...
  if [ ! -f "${CODE_DIR}/docker/${DOCKER_MODE}/${OS_NAME}${OS_VER}.sh" ]
  then
    echo "ERR: ${CODE_DIR}/docker/${DOCKER_MODE}/${OS_NAME}${OS_VER}.sh not Found"
    exit 1
  fi
}
